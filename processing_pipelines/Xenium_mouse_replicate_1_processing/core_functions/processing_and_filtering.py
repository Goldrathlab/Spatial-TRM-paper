...
def plot_qc_feature(cell_by_gene, cell_meta, path, control_probes=False):
    ...
    if control_probes:
        ...
        metrics = ['total_transcripts', 'nuclear_transcripts', 'cytoplasmic_transcripts','nuclear_transcript_percentage', 'control_probe_counts']
        ncols = 5
    else:
        colors = ['#98FB98', '#E6E6FA', '#ADD8E6', '#FFB6C1']
        metrics = ['total_transcripts', 'nuclear_transcripts', 'cytoplasmic_transcripts','nuclear_transcript_percentage']
        ncols = 4
...
def qc_before_clustering(
    ...
    adata = adata[(adata.obs['nuclear_transcripts'] > min_nuclear_transcripts) & (adata.obs['nuclear_transcripts'] < max_nuclear_transcripts), :]
    adata = adata[(adata.obs['cytoplasmic_transcripts'] > min_cyto_transcripts) & (adata.obs['cytoplasmic_transcripts'] < max_cyto_transcripts), :]
    adata = adata[(adata.obs['nuclear_transcript_percentage'] > min_nuc_pct) & (adata.obs['nuclear_transcript_percentage'] < max_nuc_pct), :]
