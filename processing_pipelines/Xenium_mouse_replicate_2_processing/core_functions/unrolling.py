...
def identify_spiral(adata, points, top_points, mid_points, num_points = 100000):
    ...
    spiral_main_top = [np.array(i)*adata.uns['unrolling_downsize'] for i in top_points]
    spiral_main_mid = [np.array(i)*adata.uns['unrolling_downsize'] for i in mid_points]
    ...
    for k in spiral_main_bottom:
        ...
        num_points = int(base_num_points*(len(k)/np.sum([np.shape(l)[0] for l in spiral_main_bottom])))
    ...
    for k in spiral_main_mid:
        ...
        num_points = int(base_num_points*(len(k)/np.sum([np.shape(l)[0] for l in spiral_main_mid])))
    ...
    for k in spiral_main_top:
        ...
        y=k[:, 1]
        # Number of points you want to evenly space
        num_points = int(base_num_points*(len(k)/np.sum([np.shape(l)[0] for l in spiral_main_top])))
    ...
    return x_points_bottom, y_points_bottom, x_points_mid, y_points_mid, x_points_top, y_points_top

def get_distances_and_indices(adata, dont_remove, x_points_bottom, y_points_bottom, x_points_mid, y_points_mid, x_points_top, y_points_top, base_num_points = 100000):
    ...
    return all_points, distances_top, indices_top, distances_bottom, indices_bottom, distances_mid, indices_mid 